...
class User_model extends CI_Model
{
    ...
    function employeeListing($searchText = '', $page, $segment){
        $this->db->select('emp.nro_legajo_emp, BaseTbl.name, emp.estado_civil_emp, emp.experiencia_emp, emp.direccion_emp, emp.salario_emp, emp.estudios_emp, emp.horas_sem_emp,Role.role');
          $this->db->from('empleado as emp');
          $this->db->join('tbl_users as BaseTbl', 'BaseTbl.userId = emp.user_emp_fk','left');
          $this->db->join('tbl_roles as Role', 'Role.roleId = BaseTbl.roleId','left');
          if(!empty($searchText)) {
    ...
    function ventasListing($searchText = '', $page, $segment){
$this->db->select('cli.id_cli, cli.nombre_cli, cli.apellido_cli, cli.dni_cli, cli.telefono_cli, p.card_num_p, p.card_name_p, cart.total');
  $this->db->from('cliente as cli');
  $this->db->join('pagos as p', 'cli.user_id_fk = p.user_id_fk','left');
  $this->db->join('total_cart as cart', 'cart.user_id_fk = cli.user_id_fk','left');

  if(!empty($searchText)) {
      $likeCriteria = "(cli.nombre_cli  LIKE '%".$searchText."%')";
      $this->db->where($likeCriteria);
  }
  $this->db->limit($page, $segment);
  $query = $this->db->get();
  ...
  return $result;
    ...
    function getPiezaCategoria()
    {
        $this->db->select('id_cat, nombre_cat');
        $this->db->from('piezas_categoria');
        $query = $this->db->get();
     ...
     */
    function addNewUser($userInfo, $empleadoInfo)
    {
      if($userInfo['roleId'] == 1 || $userInfo['roleId'] == 2){
        ...
        $empleadoInfo['user_emp_fk'] = $insert_id;
        $this->db->insert('empleado', $empleadoInfo);
     ...
     * @param number $userId : This is user id
     */
    function editUser($userInfo, $userId, $empleadoInfo)
    {
        if($userInfo['roleId'] == 1 || $userInfo['roleId'] == 2){
        $this->db->where('userId', $userId);
  ...
  $empleadoInfo['user_emp_fk'] = $userId;
    $this->db->update('empleado', $empleadoInfo);
}
        return TRUE;
    }
    ...
    function addNewTasks($taskInfo)
    {
        ...
        function addNewPiezas($PiezasInfo)
    {
        $this->db->trans_start();
        $this->db->insert('piezas_electronicas', $PiezasInfo);
    ...
    function editPieza($PiezaInfo, $piezaId)
    {
        $this->db->where('id_pie', $piezaId);
        $this->db->update('piezas_electronicas', $PiezaInfo);
    ...
    function deletePieza($piezaId)
    {
        $this->db->set('estaBorrado', 1);
        $this->db->where('id_pie', $piezaId);
