...
class User extends BaseController
{
    ...
    public function index()
    {
        ...
        {
            $this->session->set_flashdata('error', 'Por favor, cambie su contraseña primero por seguridad.');
    ...
    function updateUser()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(empty($resultPas))
                {
                $this->session->set_flashdata('nomatch', 'La contraseña no concuerda');
                redirect('userEdit');
                }
                else
                {
            ...
            if($result == true)
            {
                $process = 'Actualizando información de usuario';
                $processFunction = 'User/updateUser';
                $this->logrecord($process,$processFunction);

                $this->session->set_flashdata('success', 'La configuración de su cuenta se ha realizado correctamente');
            }
            else
            {
                $this->session->set_flashdata('error', 'Error al intentar realizar la configuración de la cuenta');
            }
    ...
    {
        $this->global['pageTitle'] = 'Cambiar contraseña';
            ...
            else
            {
                ...
                if($result > 0) {
                    ...
                    $this->logrecord($process,$processFunction);
                     ...
                     $this->session->set_flashdata('success', 'Cambio de contraseña exitoso');
                     }
                else {
                     $this->session->set_flashdata('error', 'Error al intentar cambiar de contraseña');
                    }
    ...
    {
        $this->global['pageTitle'] = '404 - Página no encontrada';
    ...
    function endTask($taskId)
    {
            ...
            if ($result > 0) {
                 $process = 'Finalizando tarea';
                 $processFunction = 'User/endTask';
                 $this->logrecord($process,$processFunction);
                 $this->session->set_flashdata('success', 'Tarea completada exitosamente');
                ...
                $this->session->set_flashdata('error', 'No se completó la tarea');
                if ($this->role != ROLE_EMPLOYEE){
                    redirect('tasks');
                 }
                 else{
...
function epiezas(){
  $data['piezaRecords'] = $this->user_model->getPiezas();
              ...
              $process = 'Ver piezas';
              $processFunction = 'User/epiezas';
              $this->logrecord($process,$processFunction);

              $this->global['pageTitle'] = 'Todas las piezas';

                $this->loadViews("epiezas", $this->global, $data, NULL);
}
...
function editPieza()
{
    $this->load->library('form_validation');
            ...
            $this->form_validation->set_rules('discount','Descuento de pieza','required');
        ...
        if($result > 0)
        {
            ...
            $processFunction = 'user/editPieza';
            $this->logrecord($process,$processFunction);
